"""Module to put any functions that are referred to in the "callables" section of Volreg.yaml"""
...
def md1d_file_callable(output_dir, inputs, stdout, stderr):
    outputs = _list_outputs(
        output_dir=output_dir, inputs=inputs, stdout=stdout, stderr=stderr
    )
    return outputs["md1d_file"]
...
def oned_file_callable(output_dir, inputs, stdout, stderr):
    ...
    return outputs["oned_file"]


def oned_matrix_save_callable(output_dir, inputs, stdout, stderr):
    outputs = _list_outputs(
        output_dir=output_dir, inputs=inputs, stdout=stdout, stderr=stderr
    )
    return outputs["oned_matrix_save"]
