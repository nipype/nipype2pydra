"""Module to put any functions that are referred to in the "callables" section of Hist.yaml"""
...
def out_show_callable(output_dir, inputs, stdout, stderr):
    outputs = _list_outputs(
        output_dir=output_dir, inputs=inputs, stdout=stdout, stderr=stderr
    )
    return outputs["out_show"]
...
# Original source at L1572 of <nipype-install>/interfaces/afni/preprocess.py
def _list_outputs(inputs=None, stdout=None, stderr=None, output_dir=None):
    ...
    if not inputs.showhist:
        outputs["out_show"] = attrs.NOTHING
    return outputs
